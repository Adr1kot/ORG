...
ESX.RegisterServerCallback('esx_organisation:getSocietyMoney', function(source, cb, societyyy)
	if society then
		TriggerEvent('esx_addonaccount:getSharedAccount', 'society_'..societyyy, function(account)
	...
	if price and type(price) == 'number' then
		TriggerEvent('esx_addonaccount:getSharedAccount', 'society_'..society, function(account)
			if price and account.money and  account.money >= price then
				...
				TriggerEvent('esx:showAdvancedNotification', 'System', notif.empty, '~r~Niewystarczająca kwota pieniędzy na koncie organizacji!', notif.icon, 7)
			end
		end)
	else
		if weapon:find('item_' ) then
...
AddEventHandler('esx_organisation:withdrawMoney', function(society, amount, typemoney)
	...
	TriggerEvent('esx:showAdvancedNotification', 'System', notif.empty, xPlayer.source, 'Pobrałeś z konta firmy: ~r~'..ESX.Math.GroupDigits(amount)..'$~s~', notif.icon, 7)
...
AddEventHandler('esx_organisation:depositMoney', function(society, amount)
	...
	TriggerEvent('esx:showAdvancedNotification', 'System', notif.empty, xPlayer.source, 'Zdeponowałeś na konto firmy ~g~'..ESX.Math.GroupDigits(amount)..'$~s~', notif.icon, 7)
	else
		TriggerEvent('esx:showAdvancedNotification', 'System', notif.empty, 'Nie prawidłowa kwota!', notif.icon, 7)
	end
end)
		...
		if item.count >= 0 then
			if itemcounter >= count then
				...
				--TriggerClientEvent('esx:showNotification', xPlayer.source, 'Wpłaciłeś ~g~' .. count .. 'x~w~ ' .. item.label)
										TriggerEvent('esx:showAdvancedNotification', 'System', notif.empty, xPlayer.source, 'Wpłaciłeś ~g~' .. count .. 'x~w~ ' .. item.label, notif.icon, 7)
			else
				--TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Nieprawidłowa ilość!')
						TriggerEvent('esx:showAdvancedNotification', 'System', notif.empty, 'Nieprawidłowa ilość!', notif.icon, 7)
...
TriggerEvent('cron:runAt', 18, 0, SendDailyCash)